...
│         │                                                 │
│         ▼                                                 │
│  ThrottlingMiddleware  ──────►  RateLimiter (in-memory)   │
│         │                                                 │
│         ▼                                                 │
...
| `Pipeline/Middlewares/` | `ErrorHandling`, `Logging`, `Throttling`, `Session`, `Flow` middleware                              |
...
```
ErrorHandling → Logging → Throttling → Session → Flow → Router → Handler
```
...
Защита от флуда через `System.Threading.RateLimiting`:

- Sliding Window алгоритм (гладкое ограничение)
- Per-user лимиты (каждый UserId имеет свой счётчик)
- Whitelist для админов
- Автоматический ответ при превышении лимита
...
```json
{
  "Throttling": {
    ...
    "ThrottleMessage": "⚠️ Слишком много сообщений.",
    "WhitelistedUserIds": []
  }
}
```
